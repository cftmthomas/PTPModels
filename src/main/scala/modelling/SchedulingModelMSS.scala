...
/**
  * Scheduling model with Maximal Selection Search.
  *
  * @author Charles Thomas (cftmthomas@gmail.com)
  */
object SchedulingModelMSS extends CPModel with App {
  ...
  val solPath = outputPath + "/solutions/SCHEDMSS/" + istName + "_sol.json"
  val logPath = outputPath + "/logs/SCHEDMSS-" + istName + ".log"
  ...
  for(i <- allPatient.indices) {
    ...
    add(visited(p) <= allVehicle(i)) // Real vehicle assigned
...
//      println()
  ...
  val decVars = (for (p <- 0 until nPatient) yield {
    ...
    val decisionVar = visited(p)

    val (pvehicles, pstarts, pends) = (for(i <- indices) yield (allVehicle(i), allStart(i), allEnd(i))).unzip3
    val optionalVars = pvehicles ++ pstarts ++ pends

    val valHeuris = (i: Int) => {
      if(i < pvehicles.length) pvehicles(i).minBy(vehicleUsage)
      else{
        if(optionalVars(i).max <= patientRdv(p)) optionalVars(i).max
  ...
  //Search heuristic:
  val conflictOrderingOptionalDecisionSearch: Branching = maxSelection(
    decVars,
  ...
  def lns(time: Int): Unit = {
    ...
    //Starting LNS:
    searchEngine.search(randomPatientRelax, conflictOrderingOptionalDecisionSearch, time * 1000000000L, nIters, relaxSize, nBkts)
  }
